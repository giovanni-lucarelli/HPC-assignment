...
\begin{frame}{Thread Scaling: touch-first policy}
\begin{figure}
    \centering{
...
\begin{frame}{Thread Scaling: touch-by-all policy}
\begin{figure}
    \centering{
        \resizebox{0.9\linewidth}{!}{\input{images/threads_speed_eff_tba.pgf}}%
    }
\end{figure}
\end{frame}
...
\begin{frame}[fragile]{Strong Scaling (1/4)}
    \begin{columns}[T]
        \begin{column}{0.35\textwidth}
            \vfill
            \lstset{basicstyle=\ttfamily\scriptsize, language=bash}
...
\begin{frame}{Strong Scaling (1/4)}
    \begin{figure}
    \centering{
...
\begin{frame}[fragile]{Strong Scaling (1/4): Analysis}
    \begin{itemize}
        \item 8 MPI tasks per node $\rightarrow$ one per \textbf{NUMA region}
        \item larger number of nodes $\rightarrow$ larger number of tasks  ($8\rightarrow 128$) $\rightarrow$ smaller (local) grid size
        \item for 16 nodes (128 tasks as $16 \times8$), size $N_x = N_y = 2^{14}$ 
        \begin{equation*}
            mem_{128} = \frac{2^{14}}{16}\times\frac{2^{14}}{8}\times 16 \text{B}\approx 33.5 \text{MB}
        \end{equation*}
    \end{itemize}
...
[glucarel@lrdn4293 HPC-leonardo]$ lscpu | egrep 'L1d|L1i|L2|L3'
    ...
    \begin{center}
        \textbf{\alert{Superlinearity}} from cache effects expoitation!?\footnote{\scriptsize{If the grid is small enough}}
    \end{center}
\end{frame}

\begin{frame}[fragile]{Strong Scaling (2/4)}
    \lstset{basicstyle=\ttfamily\scriptsize, language=bash}
    \begin{lstlisting}
    GRID_SIZE_X=16384
    OMP_THREADS=112
...
\begin{frame}[fragile]{Strong Scaling (2/4): Analysis}
    \begin{itemize}
        \item 1 MPI tasks per node $\rightarrow$ lower number of tasks ($1\rightarrow 16$)
        \item for 16 nodes (16 tasks as $4\times4$), size $N_x=X_y = 2^{14}$
        \begin{equation*}
            mem_{16} = \frac{2^{14}}{4}\times \frac{2^{14}}{4}\times 16 \text{B}\approx 268 \text{MB}
        \end{equation*}
    \end{itemize}

\end{frame}

\begin{frame}[fragile]{Strong Scaling (3/4)}
    \lstset{basicstyle=\ttfamily\scriptsize, language=bash}
    \begin{lstlisting}
    GRID_SIZE_X=32768
    OMP_THREADS=14
    ...
    \begin{figure}
    \centering{
        \resizebox{0.9\linewidth}{!}{\input{images/strong32_speed_eff.pgf}}%
    }
    \end{figure}
    
\end{frame}

\begin{frame}[fragile]{Strong Scaling (4/4)}
    \lstset{basicstyle=\ttfamily\scriptsize, language=bash}
    \begin{lstlisting}
    GRID_SIZE_X=65536
    OMP_THREADS=14
    ...
    \begin{figure}
    \centering{
        \resizebox{0.9\linewidth}{!}{\input{images/strong65_speed_eff.pgf}}%
    }
    \end{figure}
